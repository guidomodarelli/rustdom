...
Los adaptadores conservan los constructores DOM del entorno (`Blob`, `File` y `FileReader`) y convierten cuerpos para `Request`, `Response` y `fetch` nativos. El parser multipart reutiliza `@remix-run/multipart-parser` porque la implementación interna de Node consulta el global `File`, que Vitest reemplaza por el del entorno. Las señales de abort se traducen en ambos sentidos y las URLs de objetos creadas por el entorno se revocan al cerrarlo.
...
- FileReader ejecuta estado, cancelación compartida, resolución de etiquetas y conversión ordinaria de texto/binario/base64 en Rust con encoding_rs y base64-simd. Las salidas usan buffers propios Latin1 o UTF16 para evitar conversiones intermedias; el host conserva eventos, tareas, resultados del realm y el parser MIME. Backing no ordinario mantiene manejo de plataforma. Se preservan las reentradas y resultados anteriores de jsdom. Ver [validación](reports/validation/file-reader.md) y [rendimiento](reports/validation/file-reader-performance.md).
- FormData conserva la lista ordenada, índices, texto y decisiones de mutación en Rust. El constructor y la preparación de File también tienen control nativo, con fábricas de realm, helpers de elementos y representaciones de valores visibles a V8 en el host. Los iteradores consultan una posición por avance. Ver [validación](reports/validation/form-data.md).
