...
/** Native metadata liveness and cumulative byte-construction activity, not live Buffer bytes. */
export interface NativeBlobStatistics { live: number; created: number; released: number; mimeUnits: number; fileNameUnits: number; concatenations: number; hostConcatenations: number; copiedBytes: number; }
/** Constructor references retained by the native binding and released at env teardown. */
export interface NativeClassReferenceStatistics { live: number; created: number; released: number; cleanupErrors: number; }
/** FileReader controllers own only scalar state; decoded counts completed native string conversions. */
export interface NativeFileReaderStatistics { live: number; created: number; released: number; decoded: number; }
/** Native FormData list ownership and capacity; File objects remain visible to V8. */
export interface NativeFormDataStatistics { live: number; created: number; released: number; entries: number; textUnits: number; capacity: number; nameCapacity: number; }
export interface NativeRuntimeStatistics extends NativeTreeStatistics { rootHosts: NativeRootHostStatistics; slotableNames: NativeSlotableNameStatistics; slotAssignments: NativeSlotAssignmentStatistics; slotBacklinks: NativeSlotBacklinkStatistics; slotSignals: NativeSlotSignalStatistics; slotAssignmentDrivers: NativeSlotAssignmentDriverStatistics; mutationRecords: NativeMutationRecordStatistics; mutationObservers: NativeMutationObserverStatistics; mutationNotifications: NativeMutationNotificationStatistics; observerDeliveries: NativeObserverDeliveryStatistics; eventStates: NativeEventStatistics; listenerRegistries: NativeListenerStatistics; abortStates: NativeAbortStatistics; xmlParsers: NativeXmlStatistics; xmlSerialization: NativeXmlSerializationStatistics; tokenLists: NativeTokenListStatistics; dataset: NativeDatasetStatistics; rectangles: NativeRectStatistics; webStorage: NativeStorageStatistics; blobs: NativeBlobStatistics; fileReaders: NativeFileReaderStatistics; formData: NativeFormDataStatistics; napiClasses: NativeClassReferenceStatistics; traversals: { live: number; created: number; released: number; operations: number; createdOperations: number }; rangeStates: NativeRangeStatistics; rangeClones: NativeRangeStatistics; rangeExtracts: NativeRangeStatistics; }
