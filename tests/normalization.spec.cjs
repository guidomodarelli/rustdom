...
/** @param {object} engine - Real DOM engine. @returns {object} State after a synchronous CSS-error callback mutates siblings. */
function inspectReentrantNormalization(engine) {
  ...
  virtualConsole.on('jsdomError', () => {
    if (!armed) return;
    callbacks++;
    if (!mutated) { mutated = true; style.appendChild(added); }
  });
  try {
    document.head.append(style);
    style.append(first, second);
    const range = document.createRange();
    range.setStart(style, 2); range.collapse(true);
    armed = true;
    style.normalize();
