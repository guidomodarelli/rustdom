/** @file Checks that syntax-based key capture creates no VM contexts across addon reloads. */
'use strict';
...
async function main() {
  const report = { capturedAt: new Date().toISOString(), node: process.version, pass: false,
    methodology: 'Each cycle reloads the actual addon with host iterator sources unavailable and the builtin-loader hook blocked, then verifies XML and its intrinsic error. Public V8 native/detached context counters must stay at baseline before and after GC; strict arguments no longer create a new realm. No private V8 ABI, mocks or exposed context handles.', samples: [] };
  try {
    ...
    for (let cycle = 0; cycle < 24; cycle++) {
      ...
      assert.equal(beforeGc.nativeContexts, report.baseline.nativeContexts);
      assert.equal(beforeGc.detachedContexts, report.baseline.detachedContexts);
  ...
  const path = `reports/memory/${report.capturedAt.replaceAll(':', '-')}-intrinsic-arguments-contexts.json`;
