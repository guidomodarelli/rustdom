...
for (const [name, runtime] of Object.entries(runtimes)) {
  test(`should ignore inherited lifetime hooks when ordinary DOM listeners change in ${name}`, () => {
    const dom = new runtime.JSDOM('<body></body>');
    const descriptor = Object.getOwnPropertyDescriptor(Object.prototype, 'onChange');
    const trace = [];
    const callback = () => trace.push('listener');
    try {
      Object.defineProperty(Object.prototype, 'onChange', {
        configurable: true, writable: true, value: () => trace.push('inherited'),
      });
      ...
      if (descriptor) Object.defineProperty(Object.prototype, 'onChange', descriptor);
